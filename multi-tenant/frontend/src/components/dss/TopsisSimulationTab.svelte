<script lang="ts">
  ...
  import DssExplainabilityModal from './DssExplainabilityModal.svelte';
...
<!-- MODAL: EXPLAINABILITY (S7-04 MATHEMATICALLY HONEST NON-BLACK-BOX) -->
{#if selectedExplainZone && simulationResult}
  <DssExplainabilityModal
    zoneItem={selectedExplainZone}
    topsisSummary={simulationResult.topsis_summary || { ideal_positive: (simulationResult as any).ideal_positive, ideal_negative: (simulationResult as any).ideal_negative }}
    criteriaSpecs={simulationResult.criteria_specs || []}
    timeSlot={timeSlot}
    onClose={() => (selectedExplainZone = null)}
  />
{/if}
