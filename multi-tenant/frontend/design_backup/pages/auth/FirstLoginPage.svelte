<script lang="ts">
  ...
  const handleSubmit = async (e?: Event) => {
    ...
    try {
      ...
      // 2. Set transition flag so Onboarding Session 1 displays the MOVA loading animation
      ...
      // 4. Directly route to /onboarding/session-1 (transition will play on onboarding page)
      onNavigate('/onboarding/session-1');
    } catch (err: any) {
      errorMsg =
