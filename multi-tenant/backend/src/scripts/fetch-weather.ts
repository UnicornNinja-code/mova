/*
 * fetch-weather.ts
 * Manual CLI Script for Fetching Live Open-Meteo Weather Forecast for All Active Zones in TypeScript
 */
...
async function fetchZoneWeatherManual() {
  ...
  try {
    ...
    if (centroids.length === 0) {
      console.log("⚠️ Tidak ada zona aktif di database. Harap jalankan 'bun run db:seed' terlebih dahulu.");
      process.exit(0);
    }
